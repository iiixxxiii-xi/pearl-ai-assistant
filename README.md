...
| 7 | **工具注册制** | TOOLS 字典统一管理工具定义，Agent 循环与具体工具解耦；加新工具只需加一行配置，不改 `_react_system_prompt` 和 `_execute_tool` |
| 8 | **多轮检索 + 硬终止** | 最多 3 轮工具调用，超限后 force reply；Agent 在轮次内自主规划检索策略 |
| 9 | **生成自检** | Agent 输出草稿后多走一轮审核：逐条检查编造/预算/款式/语气，有问题自己修正 |
| 10 | **置信度短路** | 知识库无匹配时 LLM 诚实告知，不编造，自动跳过低置信度场景 |

### 幻觉治理
| # | 技术点 | 说明 |
|---|--------|------|
| 11 | **结构化输入** | 表单 4 维度画像降低用户输入模糊性 |
| 12 | **归因约束** | Prompt 要求标注引用的知识库条目 |
| 13 | **归因交叉验证** | 代码级解析归因行，检测引用了不存在的条目 |
| 14 | **Negative Rejection** | 知识库无相关内容正确拒答 |
| 15 | **预算硬约束** | 正则提取自然语言预算数字 → 代码层过滤库存价格，27 个测试用例覆盖各类表达，将价格约束从 Prompt 软约束升级为代码硬约束 |

### 库存与推荐
| # | 技术点 | 说明 |
|---|--------|------|
| 16 | **库存管理系统** | JSON 持久化 + 跨进程文件锁 + 原子写入（tmp→rename），多 worker 并发不坏数据；有自动化测试覆盖写入中断时旧数据完整性 |
| 17 | **珠子推荐** | LLM 结合库存自动推荐珠子，实时计算珠子成本 |
| 18 | **前端库存管理** | 新增品种/出入库/数量输入，无需编辑 JSON 或重启服务 |

### 工程化
| # | 技术点 | 说明 |
|---|--------|------|
| 19 | **流式输出 SSE** | DeepSeek stream=True → 线程转异步 → SSE 推送 → 前端打字机 |
| 20 | **Redis 检索缓存** | 检索结果缓存 5 分钟，命中直接返回跳过全链路；Redis 不可用时自动降级内存字典，已自动化测试覆盖 |
| 21 | **LangChain 检索链** | LCEL 封装 Chroma 向量存储与检索，Agent ReAct 循环自己写 |
| 22 | **ReAct 工具调度层** | 原生 Python 实现 Thought→Action→Observation 循环，LLM 自主决策；工具注册制，循环逻辑与具体工具解耦 |
| 23 | **语义对话记忆** | jieba 关键词重合度 + 时间衰减，非简单滑动窗口 |
| 24 | **全链路指标** | `/api/metrics` 查检索缓存命中率 + Agentic RAG + 采纳率 |
| 25 | **检索日志持久化** | JSONL 结构化日志，支持 bad case 回溯 |
| 26 | **RAGAS 风格评估** | LLM-as-Judge 四指标自动评分 |
| 27 | **采纳率追踪** | copy/regenerate 人机协作反馈闭环 |
| 28 | **双角色 Prompt** | 妈妈/姐姐两套 System Prompt 切换 |
| 29 | **客户画像策略** | 4 维度（性格/用途/品质/了解程度）各配策略 |
| 30 | **单元测试** | 89 个测试：预算提取 27 个（自然语言正则全覆盖）+ 库存过滤 4 个 + 年龄段映射 16 个 + 预算范围扩展 12 个 + Query 类别提取 5 个 + 知识库关键词 4 个 + 检索/API/库存/RRF/记忆 17 个 + Redis 降级 2 个 + 库存原子写入 2 个 |
| 31 | **Docker Compose** | FastAPI + PostgreSQL + Redis 一键编排 |
| 32 | **自动降级** | Redis/PG 不可用时自动回退内存/JSON，不加 Docker 也能跑 |
